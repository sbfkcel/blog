...
(function($){
				...
				if(!flag){
					if($(this).attr("ignore")==="ignore" && ( $(this).val()==="" || $(this).val()===$(this).attr("tip") )){
						if(settings.tiptype==2){
							$(this).parent().find(".Validform_checktip").removeClass().addClass("Validform_checktip").text("");
			...
			var subform=function(){
				...
				if(flag && !posting){
					...
					if(settings.ajaxPost){
						...
						$(".blue_an").attr("disabled","disabled");
	...
	$.fn.Validform.sn={
		...
		showmsg:function(msg,type,o,show){//o:{obj:当前对象, type:1=>正在检测 | 2=>通过}, show用来判断tiptype=1的情况下是否弹出信息框;
			...
			if(type==2){
				xo.parent().find(".Validform_checktip").text(msg);
				this.cssctl(xo.parent().find(".Validform_checktip"),o.type);
			}
		...
		checkform:function(obj,parentobj,tiptype,show){//show用来判断是表达提交还是blur事件引发的检测;
			...
			obj.removeClass("Validform_error").addClass("Validform_ok");
			errorobj=null;
			return true;
		},
